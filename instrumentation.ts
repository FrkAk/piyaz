 ...
 * Cloudflare target is excluded by
 * the early return: its policy is the edge-provided `cf-connecting-ip`, so
 * there is nothing to refuse. The loader itself skips
 * `NEXT_PHASE=phase-production-build`, so building an image never requires
 * the variable.
 *
