...
9. **Audit every 10 creates.** Score the last three: description 2 to 4 sentences, 2 to 4 binary criteria, all three tag dimensions plus `priority`, a category from the project's list. Fix failures with a surgical `piyaz_edit` before creating more. Drift caught at task 15 is a 30-second fix; at task 50 it is 35 rewrites.
