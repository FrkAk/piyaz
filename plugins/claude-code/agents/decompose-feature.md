...
6. **Present the plan**: the feature in a sentence, the existing categories it uses and any new one you are asking for, the foundation tasks and the capability tasks each with category, estimate, and priority, the integration edges to existing refs with a one-sentence why per edge, the edges within the feature, the tag deltas, and a gap check naming anything in the description no task covers.
