...
The ask_user_question tool requires a user attached to the session. Codex `exec`, the Claude Agent SDK without a `canUseTool` callback, policy-deny contexts, and CI environments all reject or hang on the call. When you detect headless mode (the tool errors with "no input available", "policy denied", or equivalent), do not loop and do not silently fabricate a default:
