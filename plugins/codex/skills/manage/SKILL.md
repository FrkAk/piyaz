...
1. `piyaz_project action='list'` then `action='select'`. Note `projectId`. Pass it on every subsequent call (no server-side session state).
2. `piyaz_query type='overview'` once — UNLESS:
...
3. `piyaz_analyze type='ready'`, `type='blocked'`, `type='critical_path'`, `type='plannable'`. Slim, all four. Get the lay of the land before saying anything.
...
1. `piyaz_analyze type='ready'`. All unblocked.
...
4. For each: `piyaz_task action='update' status='in_progress'` plus `piyaz_context depth='agent'`.
...
1. If not already `in_progress`, set it: `piyaz_task action='update' status='in_progress'` (preserves lifecycle history).
...
5. `piyaz_task action='update' status='done' executionRecord='...' decisions=[...] files=[...] acceptanceCriteria=[...]`. Read response `_hints` and re-call with missing fields.
...
1. `piyaz_project action='list'` plus `action='select'` if not already selected.
2. **Lead with `piyaz_analyze type='critical_path'`.** This tells the user the actual shape of remaining work. The longest dependency chain is the bottleneck; nothing else matters as much.
3. `piyaz_analyze type='ready'`. What can start now.
4. `piyaz_analyze type='blocked'`. What is stuck (and why).
5. If still nothing actionable: `piyaz_analyze type='plannable'`. Drafts ready to plan.
...
2. **Bottlenecks.** Find tasks with high downstream impact (`piyaz_analyze type='downstream'` count) that are still draft or blocked. These are leverage points. Recommend planning the highest-fan-out blocker first.
3. **Stale edges.** Sample a handful of high-degree tasks via `piyaz_query type='edges'`. Look for empty notes, outdated decisions, dependencies that no longer hold. Fix them with `piyaz_edge action='update'` or `action='remove'`.
...
Tasks with zero edges are invisible to `piyaz_analyze type='ready'` and `type='blocked'`. They appear in `plannable` but never gain context from neighbors. Run periodically (default: as part of every strategic review).

1. `piyaz_analyze type='plannable'` for the candidate pool.
2. For each candidate that does NOT show up in any `piyaz_analyze type='blocked'` reasoning AND is not on the `critical_path`, run `piyaz_query type='edges' taskId=<id>`.
...
2. Before proposing changes, **explore**. Search related tasks (`piyaz_query type='search'` by tag or title fragment), read current docs for any framework or library the task touches, check the actual codebase for what already exists. **No speculation.** Refining a task on assumptions is how vague tasks survive review.
...
4. `piyaz_task action='update'`. **Do not pass `overwriteArrays=true`** without confirmation. Default append is safe.
...
1. `piyaz_task action='create'` per artifacts §1 (full description, 2 to 4 binary ACs, three tag dimensions plus the `priority` field, category match).
2. `piyaz_edge action='create'` for dependencies. Meaningful notes (artifacts §3).
3. Verify: `piyaz_query type='edges'` on the new task.
4. **Run § F** to check if existing tasks need new edges to this one.

### Delete or cancel

- **Cancel** when the rationale is worth keeping (abandoned approach, deprioritized scope, superseded design, PR closed without merge): `piyaz_task action='update' status='cancelled' executionRecord='<rationale + what was tried>' decisions=[...]`. Then run § F.
...
- **For multi-agent dispatch, verify file-level independence.** Two tasks both editing the same file are not independent even if `piyaz_analyze type='ready'` returned both.

## Token discipline

- One `overview` fetch at session start. Cache it. Do not refetch unless something significant has changed.
- Pick the right `piyaz_context` depth: `working` for refinement, `agent` for handoff, `planning` for plan-writing, `summary` for quick health.
- For status questions, lead with `piyaz_analyze` (slim) and `piyaz_query type='search'` (slim). Do not call `overview` for routine questions.
