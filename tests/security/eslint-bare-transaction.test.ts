 ...
 */
test("eslint flags bare db.transaction outside the data layer", async () => {
...
`;
  ...
  // app/* matches the strict rules block in eslint.config.mjs (and is
  // NOT on the ignores list).
  const proc = spawn({
    cmd: [
      "bun",
      "x",
  ...
  // 3 banned shapes: db.transaction, serviceRoleDb.transaction, db.select.
