...
test(
  "GET /api/events — attaches connection and registers project subs in start(), then cleans up on abort",
  async () => {
...
test(
  "GET /api/events — dispatches reach the new connection (sanity check on attach-before-register)",
  async () => {
