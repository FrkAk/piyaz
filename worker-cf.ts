 ...
 * (`PiyazBroker`, `DOQueueHandler`) and delegates `fetch` to the
...
import { PiyazBroker } from "./lib/realtime/broker-do";
...
/**
 * Terminate the realtime WebSocket upgrade against the `PiyazBroker` Durable
...
async function handleRealtimeUpgrade(
  ...
  try {
    ...
    const client = await broker.connect(authorized.userId, env.PIYAZ_BROKER);
    await broker.registerMany(authorized.userId, keys, env.PIYAZ_BROKER);
