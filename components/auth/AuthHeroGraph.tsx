...
function drawFrame(
  ...
  for (const n of nodes) {
    ...
    if (isHollow) {
      ...
      const fill = ctx.createRadialGradient(0, 0, 0, 0, 0, n.size);
 ...
 * story as a settled static tableau (drag still works).
 *
...
export function AuthHeroGraph() {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      drawFrame(ctx, w, h, nodes, links, ripples, theme, now, !reduced);
    };
    raf = requestAnimationFrame(frame);
