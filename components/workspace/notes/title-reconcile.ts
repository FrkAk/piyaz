 ...
 * React-free, with no cache or DOM access. The editor holds the title in local
 * state; the detail query cache advances `serverTitle` on a tree rename or a
 * remote change. These predicates gate when the local state adopts the server
 * value and when a local edit is committed back.
 */
 ...
 * that has converged back to the server value has nothing to commit; clearing
 * the flag keeps it from blocking a later {@link shouldAdoptServerTitle}
 * adoption or writing the stale local title back over a server change.
 *
