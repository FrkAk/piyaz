...
CREATE TABLE "notes" (
	...
	"search_tsv" "tsvector" GENERATED ALWAYS AS (setweight(to_tsvector('english', left(coalesce(title, ''), 2000)), 'A') || setweight(to_tsvector('english', left(coalesce(body, ''), 131072)), 'B')) STORED,
