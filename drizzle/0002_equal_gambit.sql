...
CREATE INDEX "note_revisions_note_id_idx" ON "note_revisions" USING btree ("note_id","version");--> statement-breakpoint
CREATE INDEX "note_task_links_note_id_idx" ON "note_task_links" USING btree ("note_id");--> statement-breakpoint
...
-- §11 egress/compute tuning (hand-added; drizzle-kit emits neither). lz4 cuts
-- detoast CPU ~60-80% vs pglz on every note-detail read; STORAGE stays EXTENDED.
ALTER TABLE "notes" ALTER COLUMN "body" SET COMPRESSION lz4;--> statement-breakpoint
ALTER TABLE "notes" ALTER COLUMN "search_tsv" SET COMPRESSION lz4;--> statement-breakpoint
-- §11 covering tree-list index. drizzle-orm 0.45.2 has no INCLUDE builder, so
-- this index is migration-managed (absent from the push-based test DB and from
-- the schema snapshot by design). The INCLUDE columns hold the notes_member_access
-- policy predicates (visibility, created_by) so the per-row RLS check stays index-only.
CREATE INDEX "notes_tree_list_idx" ON "notes" USING btree ("project_id","folder","title") INCLUDE ("slug","type","visibility","agent_writable","locked","updated_at","created_by") WHERE deleted_at IS NULL;